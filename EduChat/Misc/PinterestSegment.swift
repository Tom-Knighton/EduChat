...
@IBDesignable public class PinterestSegment: UIControl {
    ...
    public struct TitleElement: Equatable {
        ...
        public init(title: String, selectedImage: UIImage? = nil, normalImage: UIImage? = nil, subject: Subject? = nil) {
            self.title = title
    ...
    public var titles: [Subject?] {
        get {
            return titleElements.map({ $0.subject })
        }
        set {
            _titleElements = newValue.map({ TitleElement(title: $0?.SubjectName ?? "") })
        }
    }
    ...
    public convenience init(frame: CGRect, segmentStyle: PinterestSegmentStyle, subjects: [Subject]) {
        self.init(frame: frame, segmentStyle: segmentStyle, titles: subjects)
    }
    
    public init(frame: CGRect, segmentStyle: PinterestSegmentStyle, titles: [Subject]) {
        self.style = segmentStyle
        
        self._titleElements = titles.map({ TitleElement(title: $0.SubjectName ?? "", selectedImage: nil, normalImage: nil, subject: $0)})
    ...
    private func setSelectIndex(index: Int, animated: Bool, sendAction: Bool, forceUpdate: Bool = false) {
        ...
        selectIndex = index
        if sendAction {
            valueChange?(titles[index]?.SubjectId ?? -1)
